...
void RosImuSensor::init(mc_control::MCGlobalController & controller, const mc_rtc::Configuration & config)
{
  ...
  imu_sensor_topic_ = config("ros_topic_sensor", (std::string) "/bota_ft_sensor/imu");
  ...
  {
    mc_rtc::log::info("[RosImuSensor] Body sensor {} does not exist in the robot. Creating Body Sensor...",
                      bodySensor_name_);
      ...
      mc_rtc::gui::ArrayLabel(
          ...
          [this, &controller]()
          {
            auto linearAcceleration =
                controller.controller().robot().bodySensor(bodySensor_name_).linearAcceleration().transpose();
            auto angularVelocity =
                controller.controller().robot().bodySensor(bodySensor_name_).angularVelocity().transpose();
            return std::vector<double>{linearAcceleration.x(), linearAcceleration.y(), linearAcceleration.z(),
